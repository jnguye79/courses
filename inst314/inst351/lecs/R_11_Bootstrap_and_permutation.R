...
sampling_mean_Bootstrap=function(population_vec,
                       the_sample,
                       ...
                       N_bins=20)
{
  ...
  #now Bootstrapping:
  Bootstrap_means=rep(NA,num_samples)
   for (i in 1:B_samples) {
    this_sample=sample(the_sample,
                       replace = TRUE)
    Bootstrap_means[i]=mean(this_sample)
  }
  mu_B=mean(Bootstrap_means) 
  ...
  D=as.data.frame(Bootstrap_means)
  b=(max(sample_means)-min(sample_means))/N_bins
output_plot= plot_with_normal(sample_means,
    ...
    geom_vline(xintercept=mu_B,
               color="red",
               size=1,linetype="dotted")+
    geom_vline(xintercept=mean(the_sample),
             color="red",
             size=.5,)+
...
sample_size=50
our_sample=sample(marathon_Data$Age,
                  sample_size)
sampling_results=sampling_mean_Bootstrap(population_vec=marathon_Data$Age,
                the_sample=our_sample,
...
sd(sampling_results$Bootstrap_means)
...
#let's write a function that generates
#sampling distribution: 
sampling_mean_Bootstrap_shifted=function(population_vec,
                                 the_sample,
                                 sample_size=floor(length(data)/100),
                                 num_samples=1000,
                                 B_samples=1000,
                                 N_bins=20)
{
  ...
  Bootstrap_means=Bootstrap_means-
    mean(Bootstrap_means)+mean(population_vec)
...
our_sample=sample(marathon_Data$Age,
                  sample_size)
sampling_results=sampling_mean_Bootstrap_shifted(population_vec=marathon_Data$Age,
                                         the_sample=our_sample,
                                         sample_size=sample_size,
                                         num_samples=10^4,
                                         B_samples=10^4,
...
sample_size=200
our_sample=sample(prices,
                  sample_size)
sampling_results=sampling_mean_Bootstrap_shifted(population_vec=prices,
                                                 the_sample=our_sample,
                               N_bins=50)
